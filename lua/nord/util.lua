...
function M.dimm(color)
  if type(color) == "table" then
    for key, value in pairs(color) do
      color[key] = M.dimm(value)
    end
  elseif type(color) == "string" then
    local hsluv = require("nord.hsluv")
    if color ~= "NONE" then
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/nord-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, nord.Cache
